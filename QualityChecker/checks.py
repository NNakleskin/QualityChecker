...
def check_null_fields(dialect, schema, table, column, conn_dict):
...
def max_length(dialect, schema, table, column, conn_dict):
    script = read_file_content(
...
def not_utf8(dialect, schema, table, column, conn_dict):
    script = read_file_content(
...
def check_max_tech_load_ts(dialect, schema, table, conn_dict):
    script = read_file_content(
...
def check_row_count(dialect, schema, table, conn_dict):
...
def check_pk_doubles(dialect, schema, table, conn_dict):
...
def check_most_consistent_value(dialect, schema, table, column, conn_dict):
    script = read_file_content(
...
def check_columns_length_statistics(dialect, schema, table, column, conn_dict):
    script = read_file_content(
...
def check_segmentation(dialect, schema, table, conn_dict):
...
def check_bussines_key_counts(dialect, schema, table, conn_dict):
    select_pk_path = f'{path}/sql/work_with_meta/{dialect.lower()}/select_primary_key_columns.sql'
    select_pk_script = (read_file_content(select_pk_path).
                        format(table=table,
                               schema_name=schema))
